...
const Navbar = ({ options, onSelect }) => {
  ...
  return (
    <nav className=" fixed top-4 w-full py-3">
      <div className="dropdown w-52 relative" ref={dropdownRef}>
        ...
        <ul
          className={`bg-white rounded-md shadow-md mt-2 border duration-300 transition-all absolute ${
            isOpen ? "open-dropdown" : "close-dropdown"
          }`}
        >
          {options.map((option, index) => (
            <li
              className=" my-2 px-4 bg-slate-700 text-white"
              key={index}
